...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        # begin
        # if hasattr(self, "hook_manager"):
        #     attn_weights = self.hook_manager("after_dropout", ret = attn_weights)
        # end
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [Qwen2VLVisionBlock(config, attn_implementation="eager") for _ in range(config.depth)]
        )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # print(f"image_embeds shape: {image_embeds.shape}, inputs_embeds shape: {inputs_embeds.shape}")
                ...
                first_image_token = image_token_indices.min().item()
                last_image_token = image_token_indices.max().item()
                # print(f"first_image_token_id: {first_image_token}, last_image_token_id: {last_image_token}")
                cur_image_embeds_range.append((first_image_token, last_image_token + 1))
                if hasattr(self, 'hook_logger') and self.hook_logger is not None:
                    # print("!!!hook!!!")
                    self.hook_logger.log_image_embeds_range(cur_image_embeds_range)
                # end
