...
from transformers.models.qwen2_vl.hook import HookManager
...
class MaskHookLogger(object):
    ...
    def compute_projected_attentions(self, ret):
        assert len(self.image_embed_range) > 0
        st, ed = self.image_embed_range[-1]
        image_attention = ret[:,-1,st:ed] # [b, k, d]
    ...
    def log_image_embeds_range(self, ret):
        # print(f"log_image_embeds_range: {ret}")
        self.image_embed_range.append(ret[0])
        return ret

def get_hook_logger(model, device, layer_index = 20) -> MaskHookLogger:
    init_hookmanager(model.model.language_model.layers[layer_index].self_attn)
    prs = MaskHookLogger(model.model, device)
    # print(f"Hooking model {model.model.layers[layer_index].self_attn.__class__.__name__} at layer {layer_index}")
    model.model.language_model.layers[layer_index].self_attn.hook_manager.register('after_attn_mask',
                                prs.compute_attentions_withsoftmax)
    
    model.model.hook_logger = prs
